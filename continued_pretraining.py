...
percent_int = int(config["fraction"] * 100)
split_id = config.get("data_name_base", "unknown").split("-")[-1]  # assuming split info is at the end
seed = config.get("seed", 42)

output_name = f"{config['lora_cpt_output_prefix']}{model_id_clean}-amharic_percent{percent_int}_split{split_id}_seed{seed}"
